...
### Update Notes
> `Liquid Exception: Liquid syntax error (line 10): Unknown tag 'seo' in /Users/qwtel/GitHub/qwtel.github.io/_includes/head.html, included in /_layouts/base.html`

Add `jekyll-seo-tag` to `plugins` (formerly `gems`) in your config file.
...
* Reworked the way tables work to do the right thing more often. Tables are now scrollable by default when they run out of space.
  However, smaller tables no longer span the full available width due to this change.
  For those cases, there is now the `full-table` class that will force a table to be the same size as the content.
  The `scroll-table` classes now only add `white-space: nowrap` to prevent text inside cells from wrapping, in case this
