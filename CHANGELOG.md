...
### License v7
The free version of Hydejack is now [GPL-3.0] licensed, which is a more restrictive Open Source license than MIT.
...
* You can continue to use your current version of Hydejack according to the MIT license, however:
* If you upgrade, keep the source code in a public repository and make sure you also include the new `LICENSE.md` file.
  DO NOT combine the *new code* with the *old license*.
* If you upgrade AND subsequently make changes to parts that aren't currently licensed differently,
  you are required to make those changes available to the public under a GPL-3.0 compatible license~~
...
If this change is not acceptable to you, DO NOT upgrade or consider [buying][buy] a [PRO license][PRO].

Note that the above does not constitute legal advice.

### Breaking
This is a major release, but almost all options and APIs that were described in the docs continue to work[^1].
Some names have changed and are no longer mentioned in the docs, but they are still part of the code and continue to work.

That being said, you should be aware of these breaking changes:

* The theme now uses the `jekyll-seo-tag` plugin. Without installing it, you will run into an error.
  To resolve this, make sure you use the latest `Gemfile` and `Gemfile.lock` and run `bundle install`.
...
* `image` has been renamed to `accent_image`.
...
* Changed the way tables work, so that they do the right thing more often.
  Tables are now scrollable by default, but small tables are no longer stretched to span the full width (this is due to the way CSS works)
  Setting `scroll-table` on a larger table is sill recommended, as it will set `white-space: nowrap`,
  to prevent wrapping after every word inside cells.

* Autogenerated ids for posts now look like `post-2017-01-01-my-title` instead of `post-2017/01/01/my-title`.

[^1]: The only exception are events described in the scripting chapter. They have changed from `y-push-state-*` to `hy-push-state-*`, except `y-push-state-animationend`, which has been removed. See the [docs][pstate] for more.

[pstate]: docs/7.0.0/scripts.md#registering-push-state-event-listeners

### Changes
* `color` has been renamed to `accent_color` to be consistent with the new `accent_image` key, but the `color` key continues to work.
...
* Changed how CSS code is organized.
  ...
  one containing core styles to be inlined into the page,
  the other containing those fetched asynchronously via link tag.
  ...
  These files end in `.pre.scss`.

* Most files in `_includes` have been reorganized,
  specially `head.html` and `body.html` have been broken up into smaller parts.

* No more `<style>` tags in the `body`.

* Reader views in Firefox and Safari have an easier time recognizing the main content.

* Content that is generated via JS (error pages, loading, etc...) is now cloned from `template` tags.
...
* Added "big projects". You can make a project card span the entire content width (instead of half),
...
* If a `endDate` is missing in your `resume.json`, it will render as "[startDate] -- *present*".
  You can change the wording int he new `strings.yml` file.
...
* You can now add links to external sites in the sidebar.
  Create a file like `external.md` and add a `title`, `menu`, `order` (optional) and a `redirect_to` field to the front matter, e.g.:
  ...
  to also generate a redirect page.
...
* The PRO version now has built-in support for [Tinyletter](https://www.tinyletter.com).
...
* The PRO version now includes styles for input elements, using the same CSS class names as Bootstrap.
...
* Links can now be marked for FLIP animations by adding the `flip-title` class (use for links that have
  the same content as the title of the page they are linking to). Useful when combined with the `heading` class, e.g.
  `[NOTICE](NOTICE.md){:.heading.flip-title}`.
...
### Performance
* Drastically reduced building time during development.
  ...
  For more on why inlining CSS generally is a good idea, [see this](https://varvy.com/pagespeed/inline-small-css.html).

### Design
* The default background image is no longer anti-selling the theme...

* Code blocks, math block and tables now use as much space as there is available on the screen.
  Limiting the line length makes sense for text, as it is more difficult to read a paragraph,
  when it spans the entire length on a large display,
  but it is less useful for content that is "chunked" anyway, like tables, long formulas or code.
...
* Gray text is now a little darker:
...
* When hovering over a headline, a `#` link will appears, so that readers can link to individual headlines.

* Changed the loading spinner to use a single icon instead of several animated `div`s.

* The currently active 'category' is now marked with an icon in the sidebar.
...
* Fixed a bug where the image used during the project FLIP animation wouldn't be replaced with the higher resolution image
