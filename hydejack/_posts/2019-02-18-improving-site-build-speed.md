...
Staring with version 8.3.1, you can define sidebar entries in the config file under the `menu` key like so:
...
Note that the old way of doing things still works if you upgrade to 8.3.1. Only when you set the `menu` key will Hydejack switch to the new behavior.
{:.message}
